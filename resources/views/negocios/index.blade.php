...
@section('content')
    ...
    <div class="max-w-screen-2xl w-full mx-auto px-4 sm:px-6 lg:px-8 px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <form action="{{route('business.show', 1)}}" method="get" class="flex items-center gap-5 w-full mt-12">
                            ...
                            @foreach($negocios as $negocio)
                              <tr>
                                ...
                                <td class="w-1/6 whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium text-mundoVerdeGreen sm:pl-6 hover:underline">
                                  <a href="#">{{$negocio->Nombre}}</a>
                                </td>
                                <td class="w-1/6 whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium text-mundoVerdeGreen sm:pl-6 hover:underline">
                                    <a href="#">{{$negocio->provincia->Nombre}}</a>
                                </td>
                                <td class="w-1/6 whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium text-mundoVerdeGreen sm:pl-6 hover:underline">
                                    <a href="#">{{$negocio->lugar->Nombre}}</a>
                                </td>
                                ...
                                <td class=" w-1/6 relative whitespace-nowrap py-4 pl-3 pr-4 sm:pr-6">
                                  <div class="flex items-center">
                                      <a href="{{ route('business.edit', $negocio->NegocioID) }}">
                                      ...
                                      <div>
                                        <form action="{{ route('business.destroy', $negocio->NegocioID) }}" method="POST">

                                          @csrf
                                          @method('DELETE') 

                                        <div>
                                            <button class="w-35px h-35px cursor-pointer text-gray-500 flex items-center text-sm justify-center rounded-full hover:bg-red-600 hover:text-white duration-300" type="submit" onclick="return confirm('¿Seguro que desea eliminar el negocio?')"><i class="far fa-trash-alt"></i></button>
                                        </div>
                                        </form> 
